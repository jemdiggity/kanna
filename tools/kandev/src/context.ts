...
import { resolvePorts, type KandevPorts } from "./ports";

export interface KandevTmuxContext {
  server: string;
  session: string;
}

export interface KandevContext {
  repoRoot: string;
  homeDir: string;
  isWorktree: boolean;
  worktreeName?: string;
  ...
  tmux: KandevTmuxContext;
}

export interface ResolveKandevContextInput {
  repoRoot: string;
  homeDir: string;
  env: NodeJS.ProcessEnv;
  branch: string;
...
export function resolveKandevContext(input: ResolveKandevContextInput): KandevContext {
