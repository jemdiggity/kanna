...
import { resolveKandevContext, type KandevContext } from "../context";
...
import type { KandevPorts } from "../ports";
...
export interface ExecutorInput {
  ...
  context: {
    repoRoot: string;
    tmux: KandevContext["tmux"];
    ports: Partial<KandevPorts>;
...
async function resolveDefaultContext(env: NodeJS.ProcessEnv, options: ResolveDefaultContextOptions = {}): Promise<KandevContext> {
...
function devDbTarget(context: KandevContext): DevDbTarget {
  return {
...
export function getTaskDefinition(id: string): TaskDefinition {
  ...
  if (!definition) {
    throw new Error(`Unknown kandev task: ${id}`);
  }
  return definition;
}
