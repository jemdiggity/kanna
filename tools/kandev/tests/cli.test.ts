...
function copyLauncherFixture(sourceRepoRoot: string, fixtureRepoRoot: string): void {
  ...
  cpSync(resolve(sourceRepoRoot, "tools/kandev"), resolve(fixtureRepoRoot, "tools/kandev"), {
    recursive: true,
    filter: (source) => !source.includes("/node_modules") && !source.includes("/dist")
  });
  symlinkSync("tools/kandev/bin/kd", resolve(fixtureRepoRoot, "kd"));
}
...
describe("kandev CLI", () => {
  it("exposes install-time bin entrypoints for setup scripts", () => {
    ...
    expect(pkg.bin.kandev).toBe(pkg.bin.kd);
    expect(pkg.bin["kandev-mcp"]).not.toMatch(/^\.\/dist\//);
    expect(pkg.bin["kandev-mcp"]).toBe("./bin/kandev-mcp");
    expect(existsSync(resolve(packageRoot, pkg.bin["kandev-mcp"]))).toBe(true);
    expect(lstatSync(rootWrapper).isSymbolicLink()).toBe(true);
    expect(readlinkSync(rootWrapper)).toBe("tools/kandev/bin/kd");
  });

  it("bootstraps root kd and MCP launchers from a clean repo fixture", () => {
    const packageRoot = resolve(import.meta.dirname, "..");
    const repoRoot = resolve(packageRoot, "..", "..");
    const tempRoot = mkdtempSync(join(tmpdir(), "kandev-launcher-contract-"));
    ...
    try {
      ...
      expect(existsSync(resolve(fixtureRepoRoot, "tools/kandev/node_modules"))).toBe(true);
      expect(existsSync(resolve(fixtureRepoRoot, "tools/kandev/dist/bin/kd.js"))).toBe(true);

      const mcp = spawnSync("tools/kandev/bin/kandev-mcp", [], {
        cwd: fixtureRepoRoot,
        env,
        encoding: "utf8",
        timeout: 2_000
      ...
      expect(existsSync(resolve(fixtureRepoRoot, "tools/kandev/dist/bin/kandev-mcp.js"))).toBe(true);
    } finally {
