...
async fn test_simple_prompt_returns_messages() {
    ...
    loop {
        match session.next_message().await {
            Some(Ok(msg)) => {
                message_count += 1;
                eprintln!("[test] Message #{}: type={:?}", message_count, msg_type(&msg));
    ...
    assert!(
        message_count > 0,
        "Expected at least one message, got none"
    );
...
async fn test_debug_raw_cli_output() {
    ...
    let mut child = tokio::process::Command::new(&binary)
        .args([
            "-p", "Respond with exactly one word: HELLO",
    ...
    while let Ok(result) = tokio::time::timeout(
        ...
        let type_str = serde_json::from_str::<serde_json::Value>(&line)
            ...
            .unwrap_or("???".to_string());
        eprintln!("[debug] Line {}: type={}, len={}", line_count, type_str, line.len());
