...
| `follow_task` | boolean | no | For stages that create a new task, whether the UI should select the created task. Defaults to `true`; set `false` for stages like `pr` that should leave focus on the task list. |
