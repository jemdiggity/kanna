...
pub fn apply_child_env<J>(command: &mut std::process::Command, explicit: J)
where
    J: IntoIterator<Item = (String, String)>,
{
...
mod tests {
    ...
    fn strips_inherited_kanna_and_webdriver_env() {
        let env = build_child_env_from_iter(
            [
                ...
                ("KANNA_DB_NAME".to_string(), "leaked.db".to_string()),
                (
    ...
    #[test]
    fn preserves_explicit_overrides_after_scrubbing_parent_env() {
        let env = build_child_env_from_iter(
            [
