...
function createTestDb(): DbHandle & {
  ...
  return {
    ...
    async select<T>(query: string, bindValues?: unknown[]): Promise<T[]> {
      const q = query.trim().toUpperCase();
      // Clone results to mimic real SQLite (returns fresh objects, not references)
      const clone = <U>(arr: U[]): U[] => arr.map((item) => ({ ...item as any })) as U[];

      // ── repo ──
      if (q.includes("FROM REPO") && q.includes("WHERE ID")) {
        return clone(_tables.repo.filter((r) => r.id === (bindValues as string[])[0])) as unknown as T[];
      }
      if (q.includes("FROM REPO") && q.includes("WHERE PATH")) {
        return clone(_tables.repo.filter((r) => r.path === (bindValues as string[])[0])) as unknown as T[];
      }
      if (q.includes("FROM REPO") && q.includes("WHERE HIDDEN")) {
        return clone(_tables.repo.filter((r) => r.hidden === 0).sort(
          (a, b) => new Date(b.last_opened_at).getTime() - new Date(a.last_opened_at).getTime()
        )) as unknown as T[];
      }
      if (q.includes("FROM REPO") && !q.includes("WHERE")) {
        return clone([..._tables.repo].sort(
          (a, b) => new Date(b.last_opened_at).getTime() - new Date(a.last_opened_at).getTime()
        )) as unknown as T[];
      }
      ...
      if (q.includes("FROM PIPELINE_ITEM") && q.includes("WHERE REPO_ID = ?") && q.includes("STAGE != 'DONE'")) {
        ...
        return clone(_tables.pipeline_item.filter((p) => p.repo_id === repoId && p.stage !== "done")) as unknown as T[];
      }
      if (q.includes("FROM PIPELINE_ITEM") && q.includes("WHERE REPO_ID")) {
        return clone(_tables.pipeline_item.filter((p) => p.repo_id === (bindValues as string[])[0])) as unknown as T[];
      }
      if (q.includes("FROM PIPELINE_ITEM") && q.includes("ACTIVITY = 'WORKING'")) {
        return clone(_tables.pipeline_item.filter((p) => p.activity === "working")) as unknown as T[];
      }
      if (q.includes("FROM PIPELINE_ITEM") && q.includes("STAGE = 'DONE'") && q.includes("ORDER BY")) {
        return clone(_tables.pipeline_item
      ...
      if (q.includes("FROM PIPELINE_ITEM") && q.includes("STAGE = 'BLOCKED'") && q.includes("NOT EXISTS")) {
        return clone(_tables.pipeline_item.filter((p) => {
      ...
      if (q.includes("FROM TASK_BLOCKER") && q.includes("WHERE BLOCKED_ITEM_ID")) {
        return clone(_tables.task_blocker.filter(
          (b) => b.blocked_item_id === (bindValues as string[])[0]
        )) as unknown as T[];
      }
      // JOIN queries for listBlockersForItem / listBlockedByItem
      // Disambiguate by checking WHERE clause specifically
      if (q.includes("FROM PIPELINE_ITEM") && q.includes("JOIN TASK_BLOCKER") && q.includes("WHERE TB.BLOCKED_ITEM_ID = ?")) {
        // listBlockersForItem: WHERE tb.blocked_item_id = ? → returns blocker items
        ...
        return clone(_tables.pipeline_item.filter((p) => blockerIds.includes(p.id))) as unknown as T[];
      }
      if (q.includes("FROM PIPELINE_ITEM") && q.includes("JOIN TASK_BLOCKER") && q.includes("WHERE TB.BLOCKER_ITEM_ID = ?")) {
        // listBlockedByItem: WHERE tb.blocker_item_id = ? → returns blocked items
        ...
        return clone(_tables.pipeline_item.filter((p) => blockedIds.includes(p.id))) as unknown as T[];
      }

      // ── settings ──
      if (q.includes("FROM SETTINGS")) {
        return clone(_tables.settings.filter((s) => s.key === (bindValues as string[])[0])) as unknown as T[];
      }
...
describe("sortedItemsForCurrentRepo", () => {
  ...
  it("within a stage, sorts by activity ascending (idle, unread, working)", async () => {
    db._tables.repo.push(makeRepo({ id: "r1", path: "/test/repo" }));
    // Note: init() transitions "working" → "unread", so set activities AFTER init
    db._tables.pipeline_item.push(
      ...
      makeItem({ id: "working", repo_id: "r1", stage: "in_progress", activity: "idle" }),
    );
    await initStore();
    // Set activity to "working" directly after init to avoid the stale→unread transition
    db._tables.pipeline_item.find((p) => p.id === "working")!.activity = "working";
    store.selectedRepoId = "r1";
    store.bump();
    await flushAsync();
    const sorted = store.sortedItemsForCurrentRepo;
...
describe("createItem", () => {
  it("assigns unique port offsets", async () => {
    ...
    await flushAsync(); // let items.value update before second create
